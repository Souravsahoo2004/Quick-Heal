...
const ProfilePage: React.FC = () => {
  ...
  const generateUploadUrl = useMutation(api.adminUsers.generateUploadUrl);
  const updateAdminProfilePicture = useMutation(api.adminUsers.updateAdminProfilePicture);
  const createOrUpdateAdmin = useMutation(api.adminUsers.createOrUpdateAdmin);

  // Query admin data from Convex (for profile picture)
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        try {
          const docRef = doc(firestore, "adminUsers", currentUser.uid);
          ...
          if (docSnap.exists()) {
            ...
            // Sync admin to Convex (without photo)
    ...
    if (convexAdmin && user) {
      setUser((prev) =>
        prev
          ? {
              ...prev,
              photoURL: convexAdmin.photoURL,
              photoStorageId: convexAdmin.photoStorageId,
            }
          : null
      );
    }
  ...
  const handleNameChange = async () => {
    ...
    try {
      // Update in Firestore
      const docRef = doc(firestore, "adminUsers", user.uid);
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      await updateAdminProfilePicture({
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center py-8">
      ...
      <div className="bg-white shadow rounded-xl w-full max-w-md px-6 py-8 flex flex-col items-center">
        ...
        <div className="text-xl font-medium text-gray-900 mb-2">
          {editingName ? (
            <input
              className="border-b border-gray-300 outline-none px-2 py-1"
              value={newName}
        ...
        <div className="w-full flex flex-col gap-2">
          ...
          <button
            onClick={() => router.push("/Seller/Admin_Verify/Admin_Login")}
